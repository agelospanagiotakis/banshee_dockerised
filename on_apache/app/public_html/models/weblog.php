<?php
	...
	class weblog_model extends Banshee\model {
		...
		public function comment_okay($comment) {
			$result = true;
			...
			if (trim($comment["content"]) == "") {
				...
				if (($bbcodes = $message->bbcodes_left_open()) != null) {
					$this->view->add_message("These BBcodes were not opened or closed: %s", implode(", ", $bbcodes));
					$result = false;
				}
			}
		...
		private function send_notification($comment) {
			...
			$email = new \Banshee\Protocol\email("Weblog comment posted", $this->settings->webmaster_email);
