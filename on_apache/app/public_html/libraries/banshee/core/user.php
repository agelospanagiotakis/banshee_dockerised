	...
	final class user {
		...
		public function __construct($db, $settings, $session) {
			...
			if (isset($_SERVER["HTTP_AUTHORIZATION"])) {
				...
				if (count($auth) == 2) {
					if (($auth[0] == "Basic") && (($auth[1] = base64_decode($auth[1])) !== false)) {
						list($username, $password) = explode(":", $auth[1], 2);
		...
		public function logout() {
			if (is_true(ENCRYPT_DATA)) {
				...
				$cookie->private_key = null;
				$cookie->crypto_key = null;
			}
		...
		public function has_role($role) {
			if (is_int($role)) {
				if (is_array($this->record["role_ids"])) {
