<?php
	...
	final class view extends XML {
		...
		/* Add CKEditor
		 */
		public function start_ckeditor($selector = "#editor") {
			if (is_false(USE_CKEDITOR) || (file_exists("js/banshee/ckeditor.js") == false)) {
				return;
			}

			$this->add_javascript("banshee/ckeditor.js");
			$this->add_javascript("banshee/ckeditor_start.js");
			$this->add_css("banshee/ckeditor.css");
			$this->run_javascript("ckeditor_start('".$selector."')");
		}
		...
		public function set_layout($layout = null) {
			if ($layout === null) {
				$inherit_layout = array(LOGOUT_MODULE, "password", "account");
				if (substr($this->page->url, 0, 4) == "/cms") {
					$this->layout = LAYOUT_CMS;
				} else if (in_array($this->page->module, $inherit_layout)) {
					if (isset($_SESSION["previous_layout"])) {
						if (($_SESSION["previous_layout"] ?? null) == LAYOUT_CMS) {
							$this->layout = LAYOUT_CMS;
						}
					}
				}
		...
		/* Set XSLT file
		 *
		 * INPUT:  string XSLT file
		 * OUTPUT: bool XSLT file accepted
		 * ERROR:  -
		 */
		public function set_xslt_file($xslt_file) {
			if (file_exists("../views/".$xslt_file) == false) {
				return false;
			}
		...
		public function generate() {
			...
			switch ($this->mode) {
				...
				case "xml":
					...
					break;
				case null:
					if ($this->xslt_file == null) {
						$xslt_file = "../views/".$this->page->view.".xslt";
					} else {
						$xslt_file = "../views/".$this->xslt_file;
					}
