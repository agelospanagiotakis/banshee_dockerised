<?php
	...
	class settings {
		private $db = null;
		private $cache = array();
		private $types = array("boolean", "float", "integer", "string", "variable");
		...
		public function __get($key) {
			...
			switch ($setting[0]["type"]) {
				...
				case "variable": $value = json_decode($value, true); break;
			}
		...
		public function __set($key, $value) {
			...
			if ($value === null) {
				...
				$this->store($key, "integer", (int)$value);
				...
				$this->store($key, "variable", json_encode($value));
			}
		}
