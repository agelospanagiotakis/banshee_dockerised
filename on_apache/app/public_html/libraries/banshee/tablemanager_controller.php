<?php
	...
	abstract class tablemanager_controller extends controller {
		...
		private   $_table_class = "table table-striped table-hover table-condensed tablemanager";
		...
		protected function show_overview() {
			switch ($this->browsing) {
				...
				case "pagination":
					...
					$pagination = new pagination($this->view, "tableadmin_".$this->model->table, $this->page_size, $item_count);
					...
					if ($item_count == 0) {
						$items = array();
					} else if (($items = $this->model->get_items($pagination->offset, $pagination->size)) === false) {
						$this->view->add_tag("result", "Error while creating overview.");
						return;
					}
					break;
			...
			if ($this->sortable) {
				...
				$this->view->add_javascript("banshee/tablemanager.js");
				$this->view->add_css("webui/jquery-ui.css");
			}

			/* Labels
			 */
			$this->view->open_tag("labels", array(
				"name"  => strtolower($this->name),
				"order" => show_boolean($this->model->manual_order)));
			...
			/* Values
			 */
			$this->view->open_tag("items", array("sortable" => show_boolean($this->sortable)));
			...
			switch ($this->browsing) {
				case "alphabetize":
					...
					break;
				case "pagination":
					$pagination->show_browse_links($this->pagination_links, $this->pagination_step);
					break;
			}
		...
		protected function show_item_form($item) {
			...
			foreach ($this->model->elements as $name => $element) {
				...
				if (isset($element["placeholder"])) {
					$this->view->add_tag("placeholder", $element["placeholder"]);
				}

				if ($element["type"] == "boolean") {
					$item[$name] = show_boolean($item[$name] ?? false);
		...
		public function execute() {
			if ($this->page->ajax_request) {
				if (method_exists($this->model, "save_sorting")) {
					$this->model->save_sorting($_POST["order"]);
				}
				return;
			}

			$this->view->title = $this->name." administration";
			if (file_exists("../views/".$this->page->view.".xslt") == false) {
				$this->view->set_xslt_file("banshee/tablemanager.xslt");
			}
