<?php
	...
	class cms_photo_collection_model extends Banshee\model {
		...
		public function save_okay($collection) {
			$result = true;
		...
		public function create_collection($collection) {
			...
			if ($this->set_collection_albums($this->db->last_insert_id, $collection["albums"] ?? null) == false) {
				$this->db->query("rollback");
				return false;
			}
		...
		public function update_collection($collection) {
			...
			if ($this->set_collection_albums($collection["id"], $collection["albums"] ?? null) == false) {
				$this->db->query("rollback");
				return false;
			}
