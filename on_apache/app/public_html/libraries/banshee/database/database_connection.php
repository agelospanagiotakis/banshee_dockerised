<?php
	...
	abstract class database_connection {
		const READONLY = array("select", "show", "describe", "explain");
		...
		public function query() {
			...
			if ($this->read_only) {
				...
				if (in_array($statement, self::READONLY) == false) {
			...
			if (($result = call_user_func($this->db_query, $query, $this->link)) === false) {
		...
		public function execute() {
			...
			if (in_array($statement, self::READONLY)) {
				$result = array();
		...
		public function import($filename) {
			...
			foreach ($queries as $i => $line) {
				$queries[$i] = trim($line);
			}

			$queries = explode("; ", implode(" ", $queries));
			...
			foreach ($queries as $query) {
				...
				for ($i = 0; $i < $length; $i++) {
					...
					if ($start === null) {
						if (in_array($char, $replace_keys)) {
							$start = $i;
							$quote = $char;
						}
					} else if ($char == $quote) {
						$param = substr($query, $start + 1, $i - $start - 1);
						...
						$i -= strlen($param) - strlen($replace[$quote]) + 2;
						$start = null;
					}
				}
