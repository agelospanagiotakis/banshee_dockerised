	...
	class cms_role_model extends Banshee\model {
		...
		public function save_okay($role) {
			...
			foreach ($roles as $current) {
				if (($current["name"] == $role["name"]) && ($current["id"] != ($role["id"] ?? null))) {
		...
		public function delete_role($role_id) {
			...
			if (table_exists($this->db, "forums")) {
				$query = "update forums set required_role_id=null where required_role_id=%d";
				array_unshift($queries, array($query, $role_id));
			}
