<?php
	...
	abstract class tablemanager_model extends model {
		...
		public function __construct() {
			...
			if (isset($_GET["order"])) {
				if ($this->manual_order && in_array($_GET["order"], array_keys($this->elements))) {
					if ($this->order[0] == $_GET["order"]) {
						$this->desc_order = $this->desc_order == false;
					} else {
						$this->desc_order = false;
					}

					$this->order = array_diff($this->order, array($_GET["order"]));
					array_unshift($this->order, $_GET["order"]);
				}
			}
		...
		public function __get($key) {
			switch ($key) {
				...
				case "manual_order": return $this->manual_order;
			}
		...
		public function get_items() {
			...
			if ($this->desc_order) {
				$order = preg_replace("/%S/", "%S desc", $order, 1);
			}
		 ...
		 *
		 * INPUT:  array( string key => string value[, ...] )
		 * OUTPUT: boolean item validation okay
		 * ERROR:  -
		 */
